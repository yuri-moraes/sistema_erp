...
def check_permissions(user, method, permission_to):
    ...
    for group in groups:
        permissions = User_Groups.objects.values('permission_id').filter(group_id=group['group_id']).all()
...
class EmployeesPermission (permissions.BasePermission):
    message = 'O funcionário não tem permissão para gerenciar funcionários'
...
class TaskPermission (permissions.BasePermission):
    message = 'O funcion;ario não tem permissão para gerenciar as tarefas de todos os funcionários'
